...
<form method="post" class="<?php echo $moduleclass_sfx ?>" action="<?php echo JUri::getInstance() ?>" data-freshmail2="<?php echo $control ?>">
	...
	<div class="control-group  form-group">
		...
		<div class="checkbox">
			<label class="hasTooltip" title="<?php echo $list->description ?>">
	...
	<div class="control-group">
		...
		<div class="form-group">
			...
			<div class="controls">
				<input name="<?php echo $control ?>[custom_fields][<?php echo $field->tag ?>]" type="<?php echo $field->type ?>" <?php if (!empty($stateValues['custom_fields'][$field->tag])) : ?>value="<?php echo htmlspecialchars($stateValues['custom_fields'][$field->tag]) ?>"<?php endif ?> class="input-block-level  form-control" <?php if ($field->required) : ?> required="required"<?php endif ?> />
			</div>
		</div>
		<?php endforeach ?>
		...
		<div class="form-group">
			...
			<div class="controls">
				<input name="<?php echo $control ?>[email]" type="email" <?php if (!empty($stateValues['email'])) : ?>value="<?php echo htmlspecialchars($stateValues['email']) ?>"<?php endif ?> class="input-block-level  form-control  validate-email required" required="required" />
			</div>
		</div>
